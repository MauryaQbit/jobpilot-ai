...
def create_test_job_data(**overrides: "Any") -> dict[str, "Any"]:
    ...
    default_data = {
        ...
        "job_hash": hashlib.md5(b"test_content", usedforsecurity=False).hexdigest(),
    }
    default_data.update(overrides)
    return default_data


def create_and_save_job(session: Session, **overrides: "Any") -> Job:
    ...
    Returns:
        Created and saved Job instance
    """
...
def test_company_creation(session: Session) -> None:
    """Test creating and querying Company models.

    Validates that Company instances can be created, persisted to
    the database, and retrieved with all fields intact.
    """
    company = Company(name="Test Co", url="https://test.co/careers")
    session.add(company)
    session.commit()
    session.refresh(company)

    result = session.exec(select(Company).where(Company.name == "Test Co"))
...
def test_company_unique_name(session: Session) -> None:
    ...
    """
    company1 = Company(name="Unique Co", url="https://unique1.co")
    session.add(company1)
    session.commit()

    company2 = Company(name="Unique Co", url="https://unique2.co")
    session.add(company2)
    with pytest.raises(IntegrityError):
        session.commit()


def test_job_creation(session: Session) -> None:
    """Test creating and querying Job models with Pydantic validation.

    Tests Job model creation using model_validate() to ensure
    Pydantic validation works correctly and salary fields persist.
    """
    ...
    result = session.exec(select(Job).where(Job.title == "AI Engineer"))
    retrieved = result.first()
    assert session.get(Company, retrieved.company_id).name == "AI Test Co"
    assert retrieved.salary_min == 100000
    assert retrieved.salary_max == 150000


def test_job_unique_url(session: Session) -> None:
    """Test job url uniqueness constraint.

    Verifies that attempting to create jobs with duplicate urls
    raises an IntegrityError due to unique constraint violation.
    """
    company = Company(name="Url Co", url=None)
    session.add(company)
    session.flush()
    ...
    create_and_save_job(
        ...
        title="Job1",
        description="Desc1",
        url="https://test.co/job",
        location="Remote",
    )

    # Attempt to create second job with same url
    ...
    job2 = Job.model_validate(job2_data)
    session.add(job2)
    with pytest.raises(IntegrityError):
        session.commit()
...
def test_salary_parsing(
    ...
    """Test salary parsing with various input formats.

    Validates that the salary parser correctly handles:
    ...
    """
    assert parse_salary(salary_input) == expected
