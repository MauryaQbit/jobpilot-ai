...
def test_migrations_run_once_per_process() -> None:
    with patch("jobpilot.database.migrations.command.upgrade") as upgrade:
        run_migrations()
        run_migrations()
...
def test_migration_failure_stops_startup() -> None:
    with (
        patch(
            "jobpilot.database.migrations.command.upgrade",
...
def test_sqlite_ddl_interruption_rolls_back_and_clean_retry_succeeds(tmp_path) -> None:
    ...
    assert final_revision == "a1b2c3d4e5f6"


def test_dashboard_migrates_before_starting_streamlit() -> None:
    from jobpilot.cli.app import dashboard

    with (
        patch("jobpilot.cli.app.run_migrations") as migrate,
        patch("jobpilot.cli.app.subprocess.run") as run,
    ):
...
def test_salary_migration_is_lossless_and_retryable(tmp_path) -> None:
    ...
    salaries = [json.loads(value) for value in raw_salaries]
